...
def trimmed_file(loaded_file):
    ...
    sec_set_toggle = 2
    triplicate = True
    trimmed_df = whs.clean.trim_excel(loaded_file, trim_index, sec_set_toggle)
    return trimmed_df

@pytest.fixture
def cleaned_file(trimmed_file):
    '''Gets loaded file to cleaned_df stage of main function; test effect of clean_excel().'''
    cleaned_df = whs.clean.clean_excel(trimmed_file)
...
def test_conversion(cleaned_file, converted_file):
    ...
    for loc_v in test_locs:
        assert round(converted_file.iloc[loc_v[0],loc_v[1]]) == round(cleaned_file.iloc[loc_v[0],loc_v[1]] * loc_v[2])